...
module.exports = function(server, userDataPath, actualizarClientesWebSocketCategorias) {
    // API para obtener todas las categorías
    server.get('/api/categorias', (req, res) => {
        fs.readFile(path.join(userDataPath, 'categoriasProductos.json'), 'utf8', (err, data) => {
    ...
    server.post('/api/categorias', (req, res) => {
        fs.readFile(path.join(userDataPath, 'categoriasProductos.json'), 'utf8', (err, data) => {
            ...
            fs.writeFile(path.join(userDataPath, 'categoriasProductos.json'), JSON.stringify(categorias, null, 2), (err) => {
                ...
                actualizarClientesWebSocketCategorias("categoriasInventario", categorias);
            });
        });
    });
    ...
    server.put('/api/categorias/:id', (req, res) => {
    ...
    server.delete('/api/categorias/:id', (req, res) => {
        const id = parseInt(req.params.id, 10);
        fs.readFile(path.join(userDataPath, 'categoriasProductos.json'), 'utf8', (err, data) => {
            if (err) return res.status(500).send('Error reading categorias file');
    ...
    server.post('/api/categorias/:id/subcategorias', (req, res) => {
        const id = parseInt(req.params.id, 10);
        fs.readFile(path.join(userDataPath, 'categoriasProductos.json'), 'utf8', (err, data) => {
            if (err) return res.status(500).send('Error reading categorias file');
    ...
    server.get('/api/categorias/:categoriaId/subcategorias', (req, res) => {
    ...
    server.put('/api/categorias/:categoriaId/subcategorias/:subcategoriaId', (req, res) => {
        const categoriaId = parseInt(req.params.categoriaId, 10);
    ...
    server.delete('/api/categorias/:categoriaId/subcategorias/:subcategoriaId', (req, res) => {
        const categoriaId = parseInt(req.params.categoriaId, 10);
        const subcategoriaId = parseInt(req.params.subcategoriaId, 10);
        fs.readFile(path.join(userDataPath, 'categoriasProductos.json'), 'utf8', (err, data) => {
            if (err) return res.status(500).send('Error reading categorias file');
